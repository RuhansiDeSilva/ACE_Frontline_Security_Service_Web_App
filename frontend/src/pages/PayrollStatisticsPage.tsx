...
import { adminPayrollService, type PayrollStatistics } from "@/services/adminPayrollService";
...
export default function PayrollStatisticsPage() {
  ...
  const fetchStatistics = async () => {
    ...
    try {
      ...
      if (selectedMonth) {
        try {
          const monthData = await adminPayrollService.getMonthlyStatistics(selectedMonth);
      ...
      try {
        const last12Data = await adminPayrollService.getLast12MonthsStatistics();
      ...
      if (selectedRole !== "ALL") {
        try {
          const roleData = await adminPayrollService.getStatisticsForRole(selectedRole);
